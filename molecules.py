...
class MoleculeDataset(torch.utils.data.Dataset):
    def __init__(self,dataset, name):
        ...
        if self.name == 'pre_training':
            ...
            self.train = dataset[:1600]
            self.test = dataset[1600:]
            ...
            self.train = dataset[:1500]
            self.test= dataset[1500:  ]
            self.val  = dataset[1500: ]
            ...
            self.data_all = dataset  
        elif self.name in [ "graph8c" ]:  
            self.train = dataset[:1500]
            self.test= dataset[1500: 2000  ]
            self.val  = dataset[1500: 2000]
            self.data_all = dataset  
        elif self.name in [ "graph8c", "sr16622"  ,  "sr251256" ,   "sr261034" ,   "sr281264"  ,"sr291467"  ,  "sr351668"   , "sr351899"  ,  "sr361446"  ,  "sr401224"]:  
            self.train = dataset
            self.test= dataset
            self.val  = dataset
            self.data_all = dataset  
        elif self.name in [ "cis_trans" ]:  
            self.train = dataset 
            self.test= dataset
            self.val  = dataset
            self.data_all = dataset  
        elif self.name in [ "enan" ]:  
            self.train = dataset 
            self.test= dataset
            self.val  = dataset
            self.data_all = dataset  
    ...
    def collate(self, samples):
        ...
        list_graph_support, list_label_support, list_graph_query, list_label_query, context_graph , task_id, list_subgraph_support, list_subgraph_query = map(list, zip(*samples))
 
        list_label_support= torch.stack(list_label_support)
        list_label_query= torch.stack(list_label_query)
 
        context_graph = dgl.batch(context_graph)
 
        return list_graph_support, list_label_support, list_graph_query, list_label_query, context_graph ,  task_id , list_subgraph_support, list_subgraph_query
