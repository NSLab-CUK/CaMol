...
def Fidelity(scores, targets, pos, neg):
    ...
    # pos_fidelity = ((scores.cpu() == targets.cpu()).float() - (neg.cpu() == targets.cpu()).float()).abs().mean()    # possitive is remove
    # neg_fidelity = ((scores.cpu() == targets.cpu()).float() - (pos.cpu() == targets.cpu()).float()).abs().mean()     # important nodes remains

    #pos_fidelity = torch.round(pos_fidelity.sum(), decimals= 4)
    #neg_fidelity = torch.round(neg_fidelity.sum(), decimals= 4)
